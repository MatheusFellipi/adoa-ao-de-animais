...
describe("Test update user dadas", () => {
  ...
  it("should respond with a 201 if update address", async () => {
    const res = await request(app)
      .put(`/api-v1/address/${address[0].id}`)
  ...
  it("should respond with a 204 if delete address", async () => {
    await request(app).delete(`/api-v1/address/${address[0].id}`).expect(401);
  }, 30000);
  
  it("should respond with a 204 if delete address", async () => {
    await request(app)
      .delete(`/api-v1/address/${address[0].id}`)
      .auth(token, { type: "bearer" })
      .expect(204);
  }, 30000);
