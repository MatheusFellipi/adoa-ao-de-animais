...
describe("Test create ad animals", () => {
  ...
  it("should respond with a 401 if delete create ad animal", async () => {
    const res = await request(app)
      .delete(`/api-v1/ad/animal/${ad_animal.id}/`)
      .expect(401);
    expect(res.body).not.toBeNull();
  }, 30000);

  it("should respond with a 200 if delete ad animal id", async () => {
     await request(app)
      .delete(`/api-v1/ad/animal/${ad_animal.id}/`)
      .auth(token, { type: "bearer" })
      .expect(204);
